...
# Function to generate random lecture times
def generate_lecture_times(start_date, end_date):
    lecture_durations = [1, 2, 3]  # durations in hours
    ...
    for weekday in weekdays:
        ...
        end_lecture_time = (datetime.combine(datetime.today(), current_start_time) + timedelta(hours=duration)).time()
        
        if end_lecture_time <= end_time_limit:
            lecture_times.append((weekday, current_start_time, end_lecture_time))
...
for course_id, professor_id in courses:
    lecture_times = generate_lecture_times(start_date, end_date)
    
    lecture_id = random.randint(1000000, 9999999)
    
    for lecture_time in lecture_times:
        weekday, start, end = lecture_time
        weekday = random.randint(1, 5)
        random_room_id = random.choice(room_ids)  # Select a random RoomID
        insert_sql = f"""
        INSERT INTO STUCENTR.Lecture (LECTUREID, COURSEID, PROFESSORID, ROOMID, WEEKDAY, STARTTIME, ENDTIME, STARTDATE, ENDDATE)
