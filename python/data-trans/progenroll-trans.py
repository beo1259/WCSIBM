...
def format_fixed_width(record):
    ...
    formatted_record += str(record['PROGRAMENROLLMENTID']).ljust(6)  # LocationID, left-justified, 15 characters
    formatted_record += str(record['STUDENTID']).ljust(10)            # LocationName, 19 characters
    formatted_record += str(record['PROGRAMID']).ljust(1)            # LocationName, 19 characters
...
# SQL Query
select = "SELECT * FROM STUCENTR.PROGRAMENROLLMENT"

# Execute the query
stmt = ibm_db.exec_immediate(conn, select)

# Local file for saving query results
local_file = '/home/linux1/WCSIBM/python/data-trans/output/progenroll_data.txt'
...
remote_file_path = '/z/z41780/WCS-IBM/data/progenroll_data.txt'
