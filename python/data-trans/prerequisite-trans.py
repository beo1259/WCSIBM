...
def format_fixed_width(record):
    formatted_record = ''
    formatted_record += str(record['COURSEID']).ljust(7)
    formatted_record += str(record['PREREQUISITEID']).ljust(1)
...
# SQL Query
select = "SELECT * FROM STUCENTR.PREREQUISITE"

# Execute the query
stmt = ibm_db.exec_immediate(conn, select)

# Local file for saving query results
local_file = '/home/linux1/WCSIBM/python/data-trans/output/prereq_data.txt'
...
remote_file_path = '/z/z41780/WCS-IBM/data/prereq_data.txt'
