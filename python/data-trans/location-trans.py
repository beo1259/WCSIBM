...
def format_fixed_width(record):
    ...
    formatted_record += str(record['LOCATIONID']).ljust(10)  # LocationID, left-justified, 15 characters
    formatted_record += str(record['LOCATIONNAME']).ljust(60)            # LocationName, 19 characters
...
# SQL Query
select = "SELECT * FROM STUCENTR.LOCATION"

# Execute the query
stmt = ibm_db.exec_immediate(conn, select)

# Local file for saving query results
local_file = '/home/linux1/WCSIBM/python/data-trans/output/location_data.txt'
...
remote_file_path = '/z/z41780/WCS-IBM/data/location_data.txt'
