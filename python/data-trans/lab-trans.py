...
def format_fixed_width(record):
    ...
    formatted_record += str(record['LABID']).ljust(8)  # LabID, left-justified, 15 characters
    formatted_record += str(record['COURSEID']).ljust(6)            # CourseID, 19 characters
    formatted_record += str(record['PROFESSORID']).ljust(10)          # PASS, 19 characters
    formatted_record += str(record['ROOMID']).ljust(7)  # RoomID, 25 characters
    ...
    formatted_record += str(record['ENDDATE']).ljust(11)   # EndDate, 10 characters
...
# SQL Query
select = "SELECT * FROM STUCENTR.LAB"

# Execute the query
stmt = ibm_db.exec_immediate(conn, select)

# Local file for saving query results
local_file = '/home/linux1/WCSIBM/python/data-trans/output/lab_data.txt'
...
remote_file_path = '/z/z41780/WCS-IBM/data/lab_data.txt'
